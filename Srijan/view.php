...
require_once 'functions.php';
	...
	{
		$query = "";
		if($_REQUEST['action'] == "getSlots" && isset($_REQUEST['docID'])  && $_REQUEST['docID'] != "" )
		{
			$docID = $_REQUEST['docID'];
			$query = "SELECT * from Slots WHERE docID='$docID' ORDER BY sTime";
		}
		else if($_REQUEST['action'] == "addSlot" && isset($_REQUEST['docID'])  && $_REQUEST['docID'] != "" && isset($_REQUEST['venue']) && $_REQUEST['venue'] != "" && isset($_REQUEST['sTime'])  && $_REQUEST['sTime'] != "" && isset($_REQUEST['eTime'])  && $_REQUEST['eTime'] != ""&& isset($_REQUEST['cap'])  && $_REQUEST['cap'] != "")
		{
			...
			$cap = $_REQUEST['cap'];												
			queryMysql("INSERT INTO Slots VALUES(NULL,'$docID','$venue','$cap','0','$sTime','$eTime')");
		}
		else if($_REQUEST['action'] == "setAppointment" && isset($_REQUEST['slotID'])  && $_REQUEST['slotID'] != "" && isset($_REQUEST['pID']) && $_REQUEST['pID'] != "")
		{
			...
			$pID = $_REQUEST['pID'];			
			$query1 = "SELECT * from Slots WHERE id='$slotID' AND cap > apps";
			$num=queryMysql($query1)->num_rows;
			if($num > 0)
			{
		...
		else if($_REQUEST['action'] == "getAppointments" && isset($_REQUEST['docID'])  && $_REQUEST['docID'] != "" && isset($_REQUEST['venue']) && $_REQUEST['venue'] != "")
		{
			$docID = $_REQUEST['docID'];
			$venue = $_REQUEST['venue'];			
			$query = "SELECT * from Appointments WHERE docID='$docID' AND venue='$venue' ORDER BY appNo";
		}
		else if($_REQUEST['action'] == "confirmAppointment" && isset($_REQUEST['appID'])  && $_REQUEST['appID'] != "")
		{
			$appID = $_REQUEST['appID'];
			queryMysql("UPDATE Appointments SET status = '1' WHERE id='$appID';");
		}
		else if($_REQUEST['action'] == "getAppointments" && isset($_REQUEST['pID'])  && $_REQUEST['pID'] != "")
		{
			$pID = $_REQUEST['pID'];
			$query = "SELECT * from Appointments WHERE pID='$pID' ORDER BY sTime";
		}
		...
		if($query != "")
		{
			...
			$res = [];
			for($i=0;$i<$num;$i++)
				array_push($res,$result->fetch_array(MYSQLI_ASSOC)); 
